...
const LotCard = ({
    ...
    // Calculate next bid - use API provided value or fallback to calculation
    const bidIncrement = lot.bidIncrement || Math.max(Math.ceil(lot.currentBid * 0.1), 100);
    const nextBid = lot.nextBid ?? (lot.status === "open"
    ...
    const handleWatchlist = async (e: MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            if (isWatchlisted) {
                await removeFromWatchlistMutation.mutateAsync(lot.id);
                setIsWatchlisted(false);
                toast("Removed from watchlist", {
                    description: "This lot has been removed from your watchlist.",
                });
            } else {
                await addToWatchlistMutation.mutateAsync(lot.id);
    ...
    const lotDataForModal = {
        ...
        buyersPremium: "15%",
        auction: {
            buyer_premium_percentage: buyerPremiumPercentage ?? null,
        },
    };
                    ...
                    <div className="flex flex-col sm:flex-row">
                        ...
                        <div className="flex-1 p-3 sm:p-4 flex flex-col gap-3">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2 sm:pt-0 border-t sm:border-t-0 border-border">
                                {lot.status === "closed" && lot.realizedPrice ? (
                                    <div>
                                        <p className="text-xs text-muted-foreground">Realized</p>
                                        <p className="text-base sm:text-lg font-semibold text-foreground">
                                            {formatPrice(lot.realizedPrice)}
                                        </p>
                                    </div>
                                ) : (
                                    <>
